...
def main():
    ...
    s1LI = s1.casefold()  # Convert to casefold for invariant lowercasing
    s1U = s1.upper()  # Convert to uppercase
    s1UI = s1.upper().casefold()  # Convert to invariant uppercase

    s2 = 'icioupas'
    s2L = s2.lower()
    s2LI = s2.casefold()
    s2U = s2.upper()
    s2UI = s2.upper().casefold()

    s3 = 'ICIOUPAS'
    s3L = s3.lower()
    s3LI = s3.casefold()
    s3U = s3.upper()
    s3UI = s3.upper().casefold()
    ...
    print('CompareStr')  # Case-sensitive string comparison function
    print(((s1 > s2) - (s1 < s2)))
    ...
    print('CompareText')  # Case-insensitive string comparison function
    ...
    input()  # To pause execution and wait for a newline on the console
