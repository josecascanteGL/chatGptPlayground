s1 = 'IciOuPas'
s1L = s1.lower()  # Converts s1 to lowercase
s1LI = s1.casefold()  # Another method to convert s1 to lowercase in a locale-insensitive manner
s1U = s1.upper()  # Converts s1 to uppercase
s1UI = s1.upper()  # Another method that should be locale-insensitive uppercase, Python doesn't differentiate
...
# Python does not require an explicit call to wait for input like readln in Delphi
input()